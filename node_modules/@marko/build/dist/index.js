...
exports.loadWebpackConfig = options => {
  ...
  while (!foundConfig) {
    ...
    if (fs.existsSync(configPath = path.join(currentDirectory, "webpack.config.js"))) {
...
const configBuilder = exports.configBuilder = ({
  ...
  const ENTRY_FILENAME_TEMPLATE = `[${production ? "id" : "name"}].[contenthash:8]`;
  const FILENAME_TEMPLATE = `${production ? "" : "[name]."}[contenthash:8]`;
  const NODE_ENV = production ? process.env.NODE_ENV = "production" : "development";
  ...
  const sharedAliases = () => ({
    ...
    "connect-gzip-static": useAppModuleOrFallback(APP_DIR, "connect-gzip-static")
  });
  ...
  const babelLoader = targets => ({
    ...
    options: _objectSpread(_objectSpread({}, babelConfig(targets)), {}, {
      cacheDirectory: true
    })
  });
  ...
  const sharedRules = ({
    ...
    return [{
      ...
      test: /\.s[ac]ss$/,
      use: styleLoaders(["resolve-url-loader", {
        loader: "sass-loader",
        options: {
          sourceMap: true
        }
      ...
      test: /\.styl$/,
      use: styleLoaders(["stylus-loader", ensurePkgs(APP_DIR, ["stylus-loader", "stylus"])])
    }, {
      test: file => file && !/\.([cm]?js|json|css|less|s[ac]ss|styl|wasm|marko)$/.test(file),
      use: [{
        loader: require.resolve("file-loader"),
        options: {
          name: `${FILENAME_TEMPLATE}.[ext]`,
          outputPath: path.relative(isServer ? BUILD_PATH : ASSETS_PATH, ASSETS_PATH),
        ...
        options: {
          ...
          minimizerOptions: {
            plugins: [require.resolve("imagemin-gifsicle"), require.resolve("imagemin-jpegtran"), require.resolve("imagemin-optipng"), require.resolve("imagemin-svgo")]
          }
        }
      }].filter(Boolean)
    }];
  ...
  if (production) {
    ...
    const getSharedCompressionPlugins = test => [new CompressionPlugin({
      test,
      algorithm: "gzip",
    ...
    serverPlugins = serverPlugins.concat(getSharedCompressionPlugins(/^assets/));
    clientPlugins = clientPlugins.concat(new MinifyCSSPlugin(), getSharedCompressionPlugins());
  }

  const getServerConfig = (fn = IDENTITY_FN) => fn(_objectSpread({
    name: "Server",
    target: "async-node",
    ...
    }), new InjectPlugin(() => `global.BROWSER_ENVS = [${browserEnvs.map(({
      env,
      targets
    }, index) => `{ 
                  env: ${JSON.stringify(env)}, 
                  test: ${index === browserEnvs.length - 1 ? "null" : getUserAgentRegExp({
      browsers: targets,
      allowHigherVersions: true
    })} 
                }`).join(", ")}]`), new InjectPlugin(async function () {
      const parts = [];

      if (ENTRY_IS_DIR) {
        this.cacheable(false);
        parts.push(await getRouterCode(entry, [BUILD_PATH, "**/node_modules", "**/components"], production));
      } else if (entry.endsWith(".js")) {
        parts.push(`import middleware from ${JSON.stringify(entry)}`, `global.MARKO_MIDDLEWARE = middleware`);
      } else {
        parts.push(`import template from ${JSON.stringify(entry)}`, `global.GET_ROUTE = () => ({ key: 'main', template })`);
      }
  ...
  const getBrowserConfig = (browser, fn = IDENTITY_FN) => fn(_objectSpread({
    name: `Browser-${browser.env}`,
    target: browser.targets.length ? `browserslist:${browser.targets.join(", ")}` : "web",
    ...
    plugins: [new webpack.DefinePlugin({
      ...
      ignoreOrder: true
    }), markoPlugin.browser, ...clientPlugins]
  }, sharedConfig({
    isServer: false,
    targets: browser.targets
  })), browser);

  const getBrowserConfigs = fn => browserEnvs.map(browserEnv => getBrowserConfig(browserEnv, fn));
...
function loadBrowsersLists(entry, production) {
  ...
  if (customBrowsersList) {
    const customBrowserEnvs = Object.entries(customBrowsersList).map(([env, targets]) => ({
      env,
      targets
    }));
    ...
    return production ? [{
      ...
      env: "dev",
      targets: ["last 1 Chrome versions", "last 1 Firefox versions", "last 1 Edge versions", "last 1 Safari versions"]
    }];
  }
}
    ...
    if (!checked) {
      ...
      for (const pkg of pkgs) {
        if (!resolveFrom.silent(dir, `${pkg}/package.json`)) {
          throw new Error(`To load ${path.extname(info.resource)} files you must first install the following packages: ${pkgs.join(", ")}.`);
        }
      }
    }
