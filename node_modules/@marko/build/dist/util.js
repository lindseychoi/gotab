...
const useAppModuleOrFallback = (dir, moduleName) => {
  ...
  const packagePath = resolveFrom.silent(dir, packageName) || require.resolve(packageName);
...
const getDirectoryLookup = async (cwd, ignore) => {
  const lookup = {};
  const filenames = await fastGlob(["**/*.marko"], {
    cwd,
    ignore
  });

  for (const filename of filenames) {
    lookup[fileNameToURL(filename).replace(/(^|\/)(?:index\/(?=index\.marko$))?([^/]+)?\.marko$/, "$1$2")] = path.join(cwd, filename);
  }
...
const getRouterCode = async (cwd, ignore, production) => {
  ...
  for (let key in directoryLookup) {
    const varName = "page__" + key.replace(/\//g, "__").replace(/[^a-z0-9$_]/g, "$");
    ...
    files[pathParts[pathParts.length - 1]] = {
      key,
      varName
    };
...
import $$index from ${JSON.stringify(require.resolve("./files/dir-index.marko"))};
${imports.join("\n")}
...
const buildRoute = (dir, level = 0) => {
  ...
  if (dir.dirs) {
    for (let key in dir.dirs) {
      ...
      if (paramName) {
        ...
        ifs.unshift(`if (part_${level} === ${JSON.stringify(key)}) {\n${buildRoute(childDir, level + 1)}\n${indent}}`);
        needsPart = true;
      }
    }
  }
  ...
  if (dir.files) {
    for (let key in dir.files) {
      ...
      const partMatch = `part_${level} === ${JSON.stringify(key === "index" ? undefined : key)}`;
      const paramName = getParamName(key);

      if (paramName) {
        ifs.push(`if (true) {\n${indent}  params[${JSON.stringify(paramName)}] = part_${level};\n${indent}  return { params, template:${file.varName} };\n${indent}}`);
      } else {
        ifs.unshift(`if (${partMatch}) {\n${indent}  return { params, template:${file.varName} };\n${indent}}`);
        needsPart = true;
      }
    }
  }
...
const buildStaticSite = async (options, stats) => {
  ...
  const {
    routes
  } = require(path.join(outputPath, "middleware.js"));

  if ((await fs.statSync(options.entry)).isDirectory()) {
    const cache = new Set();
    await Promise.all(Object.values(await getDirectoryLookup(options.entry, [outputPath, "**/node_modules", "**/components"])).map(async file => {
      const url = "/" + fileNameToURL(path.relative(options.entry, file)).replace(/.marko$/, "").replace(/(^|\/)index(?=\/|$)/g, "").replace(/\/$/, "");
  ...
  if (stats) {
    ...
    const serverStats = stats.stats.find(stats => stats.compilation.name.includes("Server"));
    const serverFiles = serverStats.compilation.chunks.map(chunk => chunk.files).reduce((all, next) => all.concat(next));
...
const buildStaticPage = async (url, cache, routes, outputPath) => {
  ...
  if (pathname && !host && !cache.has(pathname)) {
    ...
    const response = Object.assign(through(), {
      setHeader() {}
    ...
    await Promise.all(links.map(link => buildStaticPage(link, cache, routes, outputPath)));
  }
};
  ...
  if (match) {
    if (match[1]) {
      if (!warnedAboutParamNaming) {
        ...
        console.warn(`@marko/build: The file param convention has changed from "folder/:param.marko" to "folder/[param].marko".`);
      }
...
const fileNameToURL = path.sep === "/" ? identity : fileName => fileName.split(path.sep).join("/");
const urlToFileName = path.sep === "/" ? identity : fileName => fileName.split("/").join(path.sep);